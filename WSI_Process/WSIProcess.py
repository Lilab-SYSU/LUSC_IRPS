...
import h5py
import math
from WSI_function import isBlackPatch,isWhitePatch,initialize_hdf5_bag,savePatchIter_bag_hdf5,save_hdf5,screen_coords,to_percentiles
from WSI_class import isInLargeContour

Image.MAX_IMAGE_PIXELS = 933120000


class WSIProcess(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up=255, mthresh=7, close=0, use_otsu=False,
        ...
        print(self.level_dim[seg_level])
        print(img.shape)
    ...
    def visWSI(self, vis_level=0, color=(64,224,208), hole_color=(72,61,139), annot_color=(255, 0, 0),
        ...
        if custom_downsample > 1:
            ...
            print(
                "Custom Downsample: {}, Patching at {} x {}, But Final Patch Size is {} x {}".format(custom_downsample,
                                                                                                     ...
                                                                                                     target_patch_size,
                                                                                                     target_patch_size))
        cont_check_fn = isInLargeContour(contour=cont, patch_size=patch_size)
    ...
    def isInContours(cont_check_fn, pt, holes=None, patch_size=256):
        if cont_check_fn(pt):
            if holes is not None:
                return not WSIProcess.isInHoles(holes, pt, patch_size)
            else:
                return 1
        return 0
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=224, step_size=224, **kwargs):
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size=224, step_size=224,
        ...
        ###check point in contour area
        cont_check_fn = isInLargeContour(contour=cont, patch_size=ref_patch_size[0])
        ...
        results = pool.starmap(WSIProcess.process_coord_candidate, iterable)
        pool.close()
    ...
    def process_coord_candidate(coord, contour_holes, ref_patch_size, cont_check_fn):
        if WSIProcess.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size):
            return coord
        else:
            return None
