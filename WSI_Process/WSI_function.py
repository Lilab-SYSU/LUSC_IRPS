...
import os
import pdb
from WSI_class import Mosaic_Canvas
...
def DrawGrid(img, coord, shape, thickness=1, color=(0,0,0,255)):
    #cv2.rectangle(img, tuple(np.maximum([0, 0], coord-thickness//2)), tuple(coord - thickness//2 + np.array(shape)), (0, 128, 128, 128), thickness=thickness)
...
def PatchImg(coords_file_path,save_path,wsi_object,
             patch_level=0, patch_size=224):
    file = h5py.File(coords_file_path, 'r')
    dset = file['coords']
    coords = dset[:]
    indices=np.arange(len(coords))
    for idx in indices:
        coord=coords[idx]
        patchimg=wsi_object.wsi.read_region(coord,patch_level,(patch_size,patch_size)).convert('RGB')
        patchimg.save((save_path + '/' + str(coord[0]) + '_' + str(coord[1]) + '.jpg'))
